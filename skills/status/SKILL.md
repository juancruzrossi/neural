...
2. **List all feature directories in `.neural/wip/`.** For each feature directory, check which of these files exist:
   - `CONTEXT.md` — created by `/neural:interview`
...
3. **Determine the next step for each feature.** Apply this logic:
   - Missing `CONTEXT.md` → Next: `/neural:interview`
   - Has `CONTEXT.md`, missing `PLAN.md` → Next: `/neural:plan`
   - Has `CONTEXT.md` and `PLAN.md`, missing `REVIEW.md` → Next: `/neural:execute` or `/neural:review`
