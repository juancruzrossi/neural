...
Execute the implementation plan in `PLAN.md`. Work the tasks sequentially, one at a time, in dependency order. For each task, follow the vertical-slice TDD loop in [TDD.md](./TDD.md): one test → minimal implementation → next test. Never batch tests, never batch implementation.
...
Read [TDD.md](./TDD.md) before the first task — it defines the red→green→refactor loop, what a good test is, and when to mock. Run tasks in table order, respecting `Depends on`.
